...
.mdl-navigation__link:hover {
  color: #DFDFDF;
    ...
    {
        $p = '?p='.$this->g->in['p'];
        return join('', array_map(function ($n) use ($p) {
            if (is_array($n[1])) return $this->more_nav($n[1]);
            return '
            <a class="mdl-navigation__link'.($p===$n[1]?' is-active':'').'" href="'.$n[1].'">'.$n[0].'</a>';
        }, $this->g->nav1));
    }
    ...
    public function main() : string
    {
      ...
      <main class="demo-main mdl-layout__content">
        <div class="demo-container mdl-grid">
          ...
          <div class="demo-content mdl-color--white mdl-shadow--4dp content mdl-color-text--grey-800 mdl-cell mdl-cell--8-col">'.$this->g->out['msg'].$this->g->out['main'].'
          </div>
        </div>
      </main>';
    }
